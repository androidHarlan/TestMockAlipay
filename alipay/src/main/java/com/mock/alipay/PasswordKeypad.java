...
public class PasswordKeypad extends DialogFragment implements View.OnClickListener, PasswordKeyboard.OnPasswordInputListener,
    ...
    private void startLoading(CharSequence password) {
        ...
        if (mCallback != null) {
            mCallback.onInputCompleted(password,Verification);
        }
    }
    ...
    public void onInput(String character) {
        ...
        if (mPasswordBuffer.length() == passwordView.getPasswordCount()&&Verification==true) {
            startLoading(mPasswordBuffer);
        }else if(mPasswordBuffer.length() == passwordView.getPasswordCount()&&Verification==false){
            if (mCallback != null) {
                mCallback.onInputCompleted(mPasswordBuffer,Verification);

            }
        }
    }
    public void NeedVerification(boolean Verification){
